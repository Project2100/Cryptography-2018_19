...
\todo{This proof has major issues:
\begin{itemize}
    \item The distributions $Y$ and $Y'$ are modeled with distinct seeds and hash images from distinct RVs; however the extractor def requires the same seed on both, and a uniform distribution on the second
    \item The collision bound uses an independent uniform RV to assess the statistical distance; it is unclear how the same-seed notion of the extractor def affects the lemma's validity, but again, $Y'$ isn't explicitly specified to be completely uniform
\end{itemize}
}
