...
\begin{proof}
    ...
    \begin{align*} % AP190906: Totally original, need a peer review here
        & \left| \Pr(B = 0 \knowing C = c) - \Pr(B = 1 \knowing C = c) \right| & \\
        =& \left| \Pr(B = 0 \knowing R \xor m_B = c) - \Pr(B = 1 \knowing R \xor m_B = c) \right| & \text{(C definition)}\\
        =& \mathrlap{\frac{\left| \Pr(R \xor m_B = c \knowing B = 0) \Pr(B = 0) - \Pr(R \xor m_B = c \knowing B = 1) \Pr(B = 1) \right|}{\Pr(R \xor m_B = c)}} & \\
        & & \text{(Bayes' theorem)} \\
